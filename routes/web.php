...
Route::group(['prefix'=>'web', ['middleware' => ['XSS']], 'namespace'=>'Web'], function(){
   // for login
	...
	Route::get('login', 'RegisterController@login_user')->name('login');
    ...
    Route::get('category/{cat_id}', 'UserloginController@category_list')->name('prod_categories');

    Route::get('product/{product_id}', 'AllProductController@view')->name('view.product');
...
Route::group(['prefix'=>'', ['middleware' => ['XSS']], 'namespace'=>'Web'], function(){
	Route::get('/', 'UserloginController@userlogin')->name('userLogin');
});
